...
public class ResourceTypeCacheConfigurationReaderTest {
	...
	private ResourceTypeCacheConfigurationReader reader;
	...
	private CacheConfigurationParser cacheConfigurationParser = new CacheConfigurationParserImpl();

	@Before
	public void setUp() {
		reader = new ResourceTypeCacheConfigurationReader();
	...
	public void testComponentLoadedFromCacheWithAllFieldsSet() {
		...
		ResourceTypeCacheConfiguration config = reader.readComponentConfiguration(request,
				Collections.EMPTY_MAP, DEFAULT_TIME);
	...
	@Test
	public void testComponentEnabledInEntries() {
		resourceResolver.getResources().remove(CACHE_RESOURCE_PATH);
		requestedResource.setPath(requestedResource.getPath() + "/jcr:content");

		Map<String, CacheConfigurationEntry> entries = new HashMap<String, CacheConfigurationEntry>();
		entries.put(RESOURCE_TYPE, cacheConfigurationParser.parseEntry(RESOURCE_TYPE));

		ResourceTypeCacheConfiguration config = reader.readComponentConfiguration(request, entries,
				DEFAULT_TIME);
	...
	@Test
	public void testComponentEnabledInEntriesWithTimeSet() {
		resourceResolver.getResources().remove(CACHE_RESOURCE_PATH);

		Map<String, CacheConfigurationEntry> entries = new HashMap<String, CacheConfigurationEntry>();
		entries.put(RESOURCE_TYPE, cacheConfigurationParser.parseEntry(RESOURCE_TYPE + ":" + CONSOLE_TIME));
		...
		assertEquals(RESOURCE_TYPE, config.getResourceType());
		assertEquals(RESOURCE_TYPE, config.getResourceTypePath());
		assertEquals(CONSOLE_TIME, config.getTime());
	...
	@Test
	public void testComponentEnabledInEntriesWithTimeAndLevelSet() {
		Map<String, CacheConfigurationEntry> entries = new HashMap<String, CacheConfigurationEntry>();
		entries.put(
				RESOURCE_TYPE,
				cacheConfigurationParser.parseEntry(RESOURCE_TYPE + ":" + CONSOLE_TIME + ":"
						+ CONSOLE_CACHE_LEVEL));
		...
		assertEquals(CONSOLE_CACHE_LEVEL, config.getCacheLevel());
		assertEquals(RESOURCE_TYPE, config.getResourceType());
		assertEquals(RESOURCE_TYPE, config.getResourceTypePath());
		assertEquals(CONSOLE_TIME, config.getTime());
	...
	public void testComponentLoadedFromCacheWithIncorrectInvalidateFieldsSet() {
		...
		reader.readComponentConfiguration(request, Collections.EMPTY_MAP, DEFAULT_TIME);
	}

	@Test
	public void testComponentLoadedFromCacheWithNoFieldsSetAndOverridenByConsole() {
		cacheResource.put(CacheConstants.CACHE_ENABLED, false);
	...
	@Test
	public void testComponentLoadedFromCacheWithAllFieldsSetAndOverridenByConsole() {
		cacheResource.put(CacheConstants.CACHE_ENABLED, false);
		cacheResource.put(CacheConstants.CACHE_VALIDITY_TIME, XML_TIME);
		cacheResource.put(CacheConstants.CACHE_LEVEL, XML_CACHE_LEVEL);
		cacheResource.put(CacheConstants.CACHE_INVALIDATE_ON_SELF, XML_INVALIDATE_ON_SELF);
